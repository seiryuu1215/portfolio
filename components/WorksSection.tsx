...
const WORKS: Work[] = [
  ...
  },
  {
    ...
    differentiation: [
      ...
      'エンジニアタイプ診断（16タイプ）でスキル傾向を客観的に把握。診断結果に基づくおすすめ技術レコメンド機能も搭載',
    ],
    techStack: [
      'Next.js 16',
      'React 19',
    ...
    highlights: [
      'AI駆動開発 — Claude Code SubAgents 5体制（PM/実装/テスト/レビュー/日記）で設計〜テストを自動化。意思決定記録・開発日記の自動生成まで仕組み化',
      'Server Component + Client Component のハイブリッド設計 — データフェッチをサーバー側に集約し、APIラウンドトリップを排除。Recharts は dynamic import (ssr: false) で遅延読み込み',
      'Supabase RLS（Row Level Security）による多層アクセス制御 — 全17テーブルにRLSポリシーを適用。公開プロフィールは service_role キーで読み取り',
      'Anthropic API 連携 — 用語概要のAI補完（Claude）、長文からの用語一括抽出。利用上限を Free/Pro で差別化',
      'エンジニアタイプ診断 — 20問の質問から16タイプに分類。5軸スコア + タイプ別キャラクター + 詳細レポート。診断結果に基づくおすすめ技術レコメンドエンジン',
      'スキルシート管理 — 案件・担当工程・使用技術を登録し、PDF/公開URL/面談準備リストとして活用',
      '1,315テスト（Vitest + Playwright E2E）— ユニットテストカバレッジ + E2Eシナリオで品質を担保',
      '3段階ロールモデル（user/pro/admin）— Feature Gate パターンでPro機能（PDF・公開URL・診断詳細）を制御',
    ],
    features: [
      ...
      'エンジニアタイプ診断 — 20問→16タイプ分類 + 5軸スコア + おすすめ技術レコメンド',
