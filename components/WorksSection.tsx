...
const WORKS: Work[] = [
  {
    ...
    features: [
      'DARTSLIVEアカウント連携・スタッツ自動取得・フル同期/差分同期・統計グラフ（Recharts）・PHOENIX換算',
      '01/Cricket深掘り分析・ダーツアウト分析・スピード分析(ミス方向検出)・ブル率改善シミュレーター・CUセッション間比較',
      '曜日×時間帯ヒートマップ・COUNT-UPレーティング動的バンド・ゴールトラッキング（月次/年次/日次）',
      '7,000種以上のバレルDB・SVGベジエ曲線シミュレーター・100点スコアリングのレコメンドエンジン',
      'XP/ランクシステム（14種のXPルール・30段階）・12種のアワード',
      'OGP画像生成（Edge Runtime）・LINE Flex Messageで週次/月次レポート自動配信・リッチメニュー・練習メモ',
