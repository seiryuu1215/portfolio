...
const collections: Collection[] = [
  ...
  },
  {
    ...
    glow: C.cyanGlow,
    fields: [
      { n: 'dartId', t: 'string (FK→darts)' },
      { n: 'userId', t: 'string (FK→users)' },
      { n: 'userName', t: 'string (非正規化)' },
      { n: 'text', t: 'string' },
    ],
    subs: [],
  },
  ...
  },
  {
    ...
    glow: C.cyanGlow,
    fields: [
      { n: 'slug / title', t: 'string' },
      { n: 'content', t: 'Markdown' },
      { n: 'tags[]', t: 'string[]' },
      { n: 'isDraft / isFeatured', t: 'boolean' },
      { n: 'userId', t: 'string (FK→users)' },
    ],
    subs: [],
  },
  {
    ...
    fields: [
      { n: 'type', t: 'string' },
      { n: 'data', t: 'object' },
      { n: 'processedAt', t: 'Timestamp' },
    ],
    subs: [],
    adminOnly: true,
  },
  {
    id: 'lineConversations',
  ...
  },
  {
    ...
    glow: C.greenGlow,
    fields: [
      { n: 'firebaseUid', t: 'string' },
      { n: 'expiresAt', t: 'Timestamp' },
    ],
    subs: [],
    adminOnly: true,
  },
...
function CollectionBox({
  ...
  const totalH =
    headerH + col.fields.length * fieldH + 8 + subHeaderH + subH + (col.subs.length > 0 ? 8 : 0);
  ...
  return (
    ...
    >
      ...
      <rect
        ...
        stroke={show ? col.color : C.border}
      ...
      {col.subs.length > 0 && (
        <>
          <line
            x1={col.x + 8}
            y1={col.y + headerH + col.fields.length * fieldH + 8}
            x2={col.x + col.w - 8}
            y2={col.y + headerH + col.fields.length * fieldH + 8}
            stroke={C.border}
            strokeWidth={0.5}
          />
          <text
            x={col.x + 10}
            y={col.y + headerH + col.fields.length * fieldH + 24}
            fill={col.color}
            fontSize={9}
            fontWeight={600}
            fontFamily="'JetBrains Mono',monospace"
          ...
          {col.subs.map((s, i) => (
            <text
              key={s}
              x={col.x + 20}
              y={col.y + headerH + col.fields.length * fieldH + 40 + i * 16}
              fill={C.textMuted}
              fontSize={8.5}
              fontFamily="'JetBrains Mono',monospace"
            >
...
export default function ERDiagramDiagram() {
  ...
  return (
    <div style={{ fontFamily: "'JetBrains Mono','SF Mono',monospace" }}>
          ...
          >
            ...
            <div style={{ color: C.textMuted, fontSize: 11, lineHeight: 1.7 }}>
              <div style={{ marginBottom: 4 }}>
                <span style={{ color: C.text, fontWeight: 600 }}>Fields: </span>
                {active.fields.map((f) => f.n).join(', ')}
              </div>
              {active.subs.length > 0 && (
                <div>
                  <span style={{ color: C.text, fontWeight: 600 }}>Subcollections: </span>
                  {active.subs.join(', ')}
                </div>
              )}
              {active.adminOnly && (
                <div style={{ color: C.red, marginTop: 4, fontSize: 10 }}>
                  🔒 Admin SDK のみアクセス可（セキュリティルールで client read/write: false）
                </div>
              )}
            </div>
          </div>
      ...
      >
        {[
          { c: C.blue, l: 'Users (Core)' },
          { c: C.green, l: 'Settings & LINE' },
          { c: C.amber, l: 'Barrels' },
          { c: C.purple, l: 'Community' },
          { c: C.cyan, l: 'Content' },
          { c: C.pink, l: 'Payment' },
